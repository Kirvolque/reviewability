...
metric = OverallMovedRewriteLines()
...
def make_hunk_analysis(value: int) -> Analysis:
    return Analysis(
        ...
        metrics=MetricResults(
            [MetricValue("hunk.moved_rewrite_lines", value, MetricValueType.INTEGER)]
        ),
        score=1.0,
    )
...
def test_sums_hunk_rewrite_lines():
    ...
    assert result == MetricValue("overall.moved_rewrite_lines", 8, MetricValueType.INTEGER)
