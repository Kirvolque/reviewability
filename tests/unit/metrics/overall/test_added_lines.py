...
def test_no_hunks():
    ...
    assert result == MetricValue("overall.added_lines", 0, MetricValueType.INTEGER)


def test_single_hunk():
    result = metric.calculate([make_hunk_analysis(4)], [])
    assert result == MetricValue("overall.added_lines", 4, MetricValueType.INTEGER)


def test_multiple_hunks():
    result = metric.calculate([make_hunk_analysis(2), make_hunk_analysis(6)], [])
    assert result == MetricValue("overall.added_lines", 8, MetricValueType.INTEGER)
...
def test_hunk_missing_metric_is_skipped():
    ...
    assert result == MetricValue("overall.added_lines", 3, MetricValueType.INTEGER)
