...
def test_returns_changed_lines_for_moved_rewrite():
    ...
    assert result == MetricValue("hunk.moved_rewrite_lines", 3, MetricValueType.INTEGER)


def test_returns_zero_for_other_hunks():
    result = metric.calculate(make_hunk(3, 2, HunkRewriteKind.IN_PLACE_REWRITE))
    assert result == MetricValue("hunk.moved_rewrite_lines", 0, MetricValueType.INTEGER)
