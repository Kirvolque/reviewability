...
def make_metric_results(metrics: list[MetricValue] | None = None) -> MetricResults:
    return MetricResults(metrics or [])


def passing_rule() -> Rule:
    return Rule(severity=Severity.WARNING, check=lambda m: None)


def failing_rule(msg: str = "failure") -> Rule:
    return Rule(severity=Severity.ERROR, check=lambda m: msg)
...
def test_rule_fields():
    def check(m: MetricResults) -> str | None:
        return None
...
def test_rule_severity_warning():
    rule = Rule(severity=Severity.WARNING, check=lambda m: None)
    assert rule.severity == Severity.WARNING


def test_rule_severity_error():
    rule = Rule(severity=Severity.ERROR, check=lambda m: None)
...
def test_rule_violation_str_warning():
    rule = Rule(severity=Severity.WARNING, check=lambda m: "too big")
    violation = RuleViolation(rule=rule, message="too big")
    assert str(violation) == "[WARNING] too big"


def test_rule_violation_str_error():
    rule = Rule(severity=Severity.ERROR, check=lambda m: "way too big")
...
def test_rule_engine_no_rules():
    engine = RuleEngine([])
    violations = engine.evaluate(make_metric_results())
    assert violations == []
...
def test_rule_engine_rule_receives_metrics():
    received = []

    def capture_check(metrics: MetricResults) -> str | None:
        received.append(metrics)
        return None

    rule = Rule(severity=Severity.WARNING, check=capture_check)
    engine = RuleEngine([rule])
    mr = make_metric_results([MetricValue("x", 5, MetricValueType.INTEGER)])
    engine.evaluate(mr)
    assert len(received) == 1
    assert received[0] is mr
...
def test_rule_engine_rule_uses_metric_value():
    ...
    def check_too_many(m: MetricResults) -> str | None:
        v = m.get("hunk.lines_changed")
