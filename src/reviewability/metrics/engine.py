...
from reviewability.domain.models import Diff, FileDiff, Hunk, Move
...
class MetricEngine:
    ...
    then file metrics, then move metrics, then overall metrics. Scores are
    ...
    def run(self, diff: Diff) -> AnalysisReport:
        ...
        move_analyses = [self._build_move_analysis(m, hunk_analyses) for m in diff.moves]

        overall_results = [
            m.calculate(hunk_analyses, file_analyses, move_analyses)
    ...
    def _build_move_analysis(self, move: Move, hunk_analyses: list[Analysis]) -> Analysis:
        """Compute move metrics and score for a single ``Move``."""
        hunk_id_set = {id(h) for h in move.hunks}
        relevant = [a for a in hunk_analyses if id(a.subject) in hunk_id_set]

        results = MetricResults(
            [m.calculate(move, relevant) for m in self._registry.move_metrics()]
        )
        score = self._scorer.move_score(results)
        return Analysis(subject=move, metrics=results, score=score)
